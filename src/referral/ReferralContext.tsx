...
function useWhiskyContext() {
  ...
  return { 
    ...
    addPlugin: (plugin: any) => {
      // This would need to be implemented with the actual plugin system
...
export function ReferralProvider({
  ...
  const whiskyContext = useWhiskyContext()
  ...
  const getOnChainAddress = async () => {
    try {
      if (!owner || !whiskyContext.provider?.anchorProvider) return null
